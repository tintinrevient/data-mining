...
#hieronder volgen een aantal probeersels
#training_classes <- train_dtm[0,]#rijtje van alle features
#print(training_classes)
#fmla <- as.formula(training_classes~.)
#train_dtm$training_classes <- as.character(train_dtm$classes)
#train_dtm$training_classes <- as.factor(train_dtm$training_classes)
...
# TO DO: cross validate for mtry & ntree (caret package)

#model_random_start <- randomForest(x = train_dtm, y = train_labels, ntree = 10, mtry = 5)
ntreeTry = 50
stepFactor = 2
improve = 0.05
mtry = sqrt(length(train_dtm))
model_random <- tuneRF (x = train_dtm, y = train_labels, mtry, ntreeTry, stepFactor, improve, trace = TRUE, plot = TRUE, doBest = TRUE)
