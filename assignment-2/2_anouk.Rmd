...
train_dtm_dataframe <-as.data.frame(train_dtm)
...
# TO DO: cross validate for mtry & treesize

#check that the labels are regarded as factors. This way classificition is used instead of the regression funcitonality
train_labels <-as.factor(train_labels)

#set the parameters for the tune RF function
#TODO: tune these variables

#this should be high enough, it takes up quite some computational power. After tuning: test if it can be lower.
ntreeTry = 1500

#choose stepFactor 1.4: the lowest value with more than 1 result (most fine-grained search)
#???? what about 77?

#create a matrix to optimize the stepfactor and improve values
tune_matrix <- matrix(0, nrow = 100, ncol = 10)

#loop through the ranges of both values
for (i in 1: 10)
  for(j in 1 : 5)
    #stepFactor = 1.4
    stepFactor = i
    ...
    #perform the tuneRF function to find the optimal mtry
    #model_random <- tuneRF (x = train_dtm, y = train_labels, ntreeTry = ntreeTry, stepFactor = stepFactor, improve= improve, trace = FALSE, plot = FALSE, doBest = FALSE)
    
    #add the lowest OOB score to the matrix
    #find out how to do this
    tune_matrix[i,j] <- i; tune_matrix 
    #min(trace())
    
print(tune_matrix)    
stepFactor = 1.5
improve = 0.05
#model_random <- tuneRF (x = train_dtm, y = train_labels, ntreeTry = ntreeTry, stepFactor = stepFactor, improve= improve, trace = TRUE, plot = TRUE, doBest = TRUE)

    
    

#TO DO: tune the treesize for optimal results
