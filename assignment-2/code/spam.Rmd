...
```{r frequency_based_feature_selection, include=FALSE}
...
# Training document-term matrix for bigrams
train.dtm.bigram <- removeSparseTerms(train.dtm.bigram, 0.99)
# Test document-term matrix for bigrams
test.dtm.bigram <- DocumentTermMatrix(reviews.all[-index.train], 
                                      list(dictionary = dimnames(train.dtm.bigram)[[2]]))
...
train.dtm.bigram <- cbind(train.dtm, train.dtm.bigram)
# Test document-term matrix for unigrams and bigrams
test.dtm.bigram <- cbind(test.dtm, test.dtm.bigram)
```

```{r mutual_info_feature_selection, include=FALSE}
...
# Training document-term matrix
train.dtm.top150 <- train.dtm[, train.mi.order[1:150]]
# Test document-term matrix
test.dtm.top150 <- test.dtm[, train.mi.order[1:150]]

# Training document-term matrix
train.dtm.top200 <- train.dtm[, train.mi.order[1:200]]
# Test document-term matrix
test.dtm.top200 <- test.dtm[, train.mi.order[1:200]]

# Training document-term matrix
train.dtm.top250 <- train.dtm[, train.mi.order[1:250]]
# Test document-term matrix
test.dtm.top250 <- test.dtm[, train.mi.order[1:250]]

# Training document-term matrix
train.dtm.top300 <- train.dtm[, train.mi.order[1:300]]
# Test document-term matrix
test.dtm.top300 <- test.dtm[, train.mi.order[1:300]]
```
...
# Confusion matrix
conf.mat.logreg.bigram <- table(labels[-index.train], reviews.logreg.bigram.pred, dnn = c("actual", "predicted"))

perf.logreg.bigram <-performance(conf.mat.logreg.bigram) 
perf.logreg.bigram %>% kable()
```

```{r logistic_regression_top50, include=FALSE}
# Logistic regression with lasso penalty
reviews.glmnet.top50 <- cv.glmnet(as.matrix(train.dtm.top50), labels[index.train], 
                                  family="binomial", type.measure="class")
# Cross-validation on lambda
plot(reviews.glmnet.top50)

# coef(reviews.glmnet.top50, s="lambda.1se")

# Make predictions on the test set
reviews.logreg.top50.pred <- predict(reviews.glmnet.top50, newx=as.matrix(test.dtm.top50), 
                                     s="lambda.1se", type="class")

# Confusion matrix
conf.mat.logreg.top50 <- table(labels[-index.train], reviews.logreg.top50.pred, dnn = c("actual", "predicted"))

perf.logreg.top50 <-performance(conf.mat.logreg.top50) 
perf.logreg.top50 %>% kable()
```
...
```{r classifier_rpart_top50, include=FALSE}
# Grow the tree
