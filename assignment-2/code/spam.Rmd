...
result.mnb <- mnb_cv(train.dtm, 950, NA, NA)
...
df.mnb.topn %>% 
  ...
  geom_point() +
  geom_jitter() +
  scale_x_sqrt(breaks = c(50, 100, 150, 200, 250, 300, 400, 500, 700, 950, 1500, 2000)) + 
...
# Select top-n mutual-information terms from total vocabularies
index.top321 <- calculate_topn(train.dtm, topn = 321)
# Select top-n mutual-information terms from frequent terms
index.top321.freq <- calculate_topn(train.dtm.freq, topn = 321)

# Combine mi-terms and frequent terms
words.mnb.unigram <- t(bind_rows(index.top321[[2]], index.top321.freq[[2]]))
colnames(words.mnb.unigram) <- c('Top-321 mutual info', 'Top-321 frequent')
# Save to top321_unigrams.csv
...
# Confusion matrix
result.mnb.bigram <- mnb_cv(train.dtm.bigram, 2000, NA, NA)
...
# Initialize all the ys for frequency
seq.topn.freq.bigram <- seq(50, 450, by = 50)
...
df.mnb.topn.bigram %>% 
  ...
  geom_point() +
  geom_jitter() +
  scale_x_sqrt(breaks = c(50, 100, 150, 250, 300, 400, 500, 1000, 1100, 1300, 1500, 2000)) + 
  theme(legend.position = c(0.8, 0.2)) +
  xlab("Top-n terms") +
  ylab("Percent")
```
...
# Select top-n mutual-information terms from total vocabularies
index.top461.bigram <- calculate_topn(train.dtm.bigram, topn = 461)
# Select top-n mutual-information terms from frequent terms
index.top461.bigram.freq <- calculate_topn(train.dtm.bigram.freq, topn = 461)

# Combine mi-terms and frequent terms
words.mnb.bigram <- t(bind_rows(index.top461.bigram[[2]], index.top461.bigram.freq[[2]]))
colnames(words.mnb.bigram) <- c('Top-461 mutual info', 'Top-461 frequent')
# Save to top461_bigrams.csv
...
# hyper-parameter: feature selection
train.dtm.rpart <- calculate_cv(train.dtm, 950, NA, NA)
cp.rpart <- 0.036
minsplit.rpart <- 15
minbucket.rpart <-5
...
reviews.rpart.pruned <- prune(reviews.rpart, cp=0)
