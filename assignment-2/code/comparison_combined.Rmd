...
# Training document-term matrix for bigrams and unigrams together
...
test_dtm2 <- read_csv(file = 'data/test_dtm2.csv')
#test_dtm2 <- read.table(file.path('data', 'test_dtm2.txt'), check.names = F)

# Option 2: create the test bigram dataset
#test_dtm2 <- gen_bigram_test(train_dtm2, test_dtm2_tmp)
#write_csv(as.data.frame(as.matrix(test_dtm2)), file.path('data', 'test_dtm2.csv'))
...
# this is the actual view from reviews.all[c(321)]
...
train_dtm2_freq <- as.matrix(train_dtm2_freq)

# Option 1: Load the existing test bigram dataset of frequency
test_dtm2_freq <- read_csv('data/test_dtm2_freq.csv')
#test_dtm2_freq <- read.table(file.path('data', 'test_dtm2_freq.txt'), check.names = F)

test_dtm2_freq <- as.matrix(test_dtm2_freq)

# Option 2: create the test bigram dataset
#test_dtm2_freq <- gen_bigram_test(train_dtm2_freq, test_dtm2_tmp)
#write_csv(as.data.frame(as.matrix(test_dtm2_freq)), file.path('data', 'test_dtm2_freq.csv'))

f <- sapply(test_dtm2_freq, is.factor)
which(f)

f <- sapply(train_dtm2_freq, is.factor)
which(f)
...
mi_top5 <- as.data.frame(index.top950[[2]][1:5]) %>%
  rownames_to_column("term") %>%
  rename(mutual_information = `index.top950[[2]][1:5]`)

mi_top5_deceptive <- as.data.frame(index.top950[[2]]) %>%
  rownames_to_column("term") %>%
  rename(mutual_information = `index.top950[[2]]`) %>%
  arrange(desc(mutual_information)) %>%
  head(5)

mi_top5_truthful <- as.data.frame(index.top950[[2]]) %>%
  rownames_to_column("term") %>%
  rename(mutual_information = `index.top950[[2]]`) %>%
...
colnames(test_dtm2)[!(colnames(train_dtm2) %in% colnames(test_dtm2))]

# Show part of mutual-info-only top-300 words
index.bigram.top2000[[2]][1:30]

mi_top52 <- as.data.frame(index.bigram.top2000[[2]][1:5]) %>%
  rownames_to_column("term") %>%
  rename(mutual_information = `index.bigram.top2000[[2]][1:5]`)

mi_top52_deceptive <- as.data.frame(index.bigram.top2000[[2]]) %>%
  rownames_to_column("term") %>%
  rename(mutual_information = `index.bigram.top2000[[2]]`) %>%
  arrange(desc(mutual_information)) %>%
  head(5)

mi_top52_truthful <- as.data.frame(index.bigram.top2000[[2]]) %>%
  rownames_to_column("term") %>%
  rename(mutual_information = `index.bigram.top2000[[2]]`) %>%
  arrange(mutual_information) %>%
  head(5)
...
predicted_mnb <-  predict.mnb(model_mnb, as.matrix(test_dtm_mi))
...
predicted_mnb_freq <-  predict.mnb(model_mnb_freq, as.matrix(test_dtm_freq))
...
conf_mnb <- table(test_labels, predicted_mnb, dnn = c("actual", "predicted"))
...
conf_mnb_freq <- table(test_labels, predicted_mnb_freq, dnn = c("actual", "predicted"))
...
predicted_mnb2 <-  predict.mnb(model_mnb2, as.matrix(test_dtm2_mi))
...
predicted_mnb2_freq <-  predict.mnb(model_mnb2_freq, as.matrix(test_dtm2_freq))
...
conf_mnb2 <- table(test_labels, predicted_mnb2, dnn = c("actual", "predicted"))
...
conf_mnb2_freq <- table(test_labels, predicted_mnb2_freq, dnn = c("actual", "predicted"))
...
glm_top5_deceptive <- coefs %>% 
  filter(term != "(Intercept)") %>% 
  arrange(desc((coefficient))) %>% 
  head(5)

glm_top5_truthful <- coefs %>% 
  filter(term != "(Intercept)") %>% 
  arrange((coefficient)) %>% 
  head(5)
...
glm_top52_deceptive <- coefs2 %>%
  arrange(desc(coefficient)) %>% 
  head(5)

glm_top52_truthful <- coefs2 %>%
  arrange(coefficient) %>% 
  head(5)
...
l_cv2<-ggplot(tidied_cv2, aes(log(lambda), MSE)) +
  ...
  geom_vline(xintercept = log(cv_lasso$lambda.min)) +
  geom_vline(xintercept = log(cv_lasso$lambda.1se), lty = 2) +
  ggtitle("Bigram and unigram") +
  theme(axis.title.y=element_blank(),
        axis.text.y=element_blank(),
        axis.ticks.y=element_blank())

ggsave(plot_grid(l_cv, l_cv2, rel_widths = c(4.2, 4)), filename = "cv_lambda.png", width = 9, height = 3)
...
top10coef %>% select(-probability) %>% kable()

top10coef2 %>% select(-probability) %>% kable() 
...
test_preds[,"feature_type"] <- "unigram"
test_preds2[, "feature_type"] <- "bigram and unigram"

test_preds_all <- bind_rows(test_preds, test_preds2)

prec_all <- ggplot(test_preds_all, aes(prediction, fill = correct)) +
  geom_bar(position = "fill") +
  facet_grid(rows = "feature_type")

recall_all <- ggplot(test_preds_all, aes(review_type, fill = correct)) +
  geom_bar(position = "fill") +
  facet_grid(rows = "feature_type") +
  theme(axis.title.y=element_blank(),
        axis.text.y=element_blank(),
...
legend <- get_legend(recall_all)

ggsave(plot_grid(prec_all + theme(legend.position = "none"), 
                 recall_all + theme(legend.position = "none"),
                 legend, nrow = 1, rel_widths = c(5, 4, 1)), 
       filename = "conf_plots.png", width = 6, height = 4)
...
predicted_tree <- predict(model_tree,
...
predicted_tree_freq <- predict(model_tree_freq,
...
conf_tree <- table(test_labels, predicted_tree, dnn = c("actual", "predicted"))
...
conf_tree_freq <- table(test_labels, predicted_tree_freq, dnn = c("actual", "predicted"))
...
predicted_tree2 <- predict(model_tree2,
...
predicted_tree2_freq <- predict(model_tree2_freq,
...
test_preds_tree <- tibble(review_type = ifelse(test_labels==1,"deceptive","truthful"),
                     ...
                     correct = ifelse(review_type == prediction, "1", "0")) %>%
  rowid_to_column("document") %>%
  mutate(feature_type = "unigram")

test_preds_tree2 <- tibble(review_type = ifelse(test_labels==1,"deceptive","truthful"),
                     prediction = ifelse(predicted_tree2==1, "deceptive", "truthful"),
                     correct = ifelse(review_type == prediction, "1", "0")) %>%
  rowid_to_column("document") %>%
  mutate(feature_type = "bigram+unigram")

test_preds_tree_all <- bind_rows(test_preds_tree, test_preds_tree2)

prec_tree <- ggplot(test_preds_tree_all, aes(prediction, fill = correct)) +
  geom_bar(position = "fill") +
  facet_grid(rows = "feature_type")

recall_tree <- ggplot(test_preds_tree_all, aes(review_type, fill = correct)) +
  geom_bar(position = "fill") +
  facet_grid(rows = "feature_type") +
  theme(axis.title.y=element_blank(),
        axis.text.y=element_blank(),
...
ggsave(plot_grid(prec_tree + theme(legend.position = "none"), 
                 recall_tree + theme(legend.position = "none"),
                 legend, nrow = 1, rel_widths = c(5, 4, 1)), 
       filename = "confplotstreemi.png", width = 6, height = 4)

conf_tree2 <- table(test_labels, predicted_tree2, dnn = c("actual", "predicted"))
...
test_preds_tree <- tibble(review_type = ifelse(test_labels==1,"deceptive","truthful"),
                     prediction = ifelse(predicted_tree_freq==1, "deceptive", "truthful"),
                     correct = ifelse(review_type == prediction, "1", "0")) %>%
  rowid_to_column("document") %>%
  mutate(feature_type = "unigram")

test_preds_tree2 <- tibble(review_type = ifelse(test_labels==1,"deceptive","truthful"),
                     prediction = ifelse(predicted_tree2_freq==1, "deceptive", "truthful"),
                     correct = ifelse(review_type == prediction, "1", "0")) %>%
  rowid_to_column("document") %>%
  mutate(feature_type = "bigram+unigram")
...
ggsave(plot_grid(prec_tree + theme(legend.position = "none"), 
                 recall_tree + theme(legend.position = "none"),
                 legend, nrow = 1, rel_widths = c(5, 4, 1)), 
       filename = "confplotstreefq.png", width = 6, height = 4)

conf_tree2_freq <- table(test_labels, predicted_tree2_freq, dnn = c("actual", "predicted"))
...
predicted_random <- predict(model_random, test_dtm_freq, type = "class")
...
# missing_bigrams <- colnames(train_dtm2_freq)[!(colnames(train_dtm2_freq) %in% colnames(test_dtm2_freq))]
# 
# newmatrix <- matrix(nrow = nrow(test_dtm2_freq),
#                     ncol = length(missing_bigrams))
# 
# 
# newmatrix <- as_tibble(newmatrix) %>% 
#   replace(is.na(.), 0) %>%
#   rename_all(funs(as.list(missing_bigrams))) %>%
#   bind_cols(test_dtm2_freq)
# 
# dim(newmatrix)
# dim(train_dtm2_freq)
# 
# newmatrix <- newmatrix[,colnames(train_dtm2_freq)]

predicted_random2 <- predict(model_random2, newdata = test_dtm2_freq, type = "class")
...
test_preds_random2 <- tibble(review_type = ifelse(test_labels==1,"deceptive","truthful"),
                     prediction = ifelse(predicted_random2==1, "deceptive", "truthful"),
                     correct = ifelse(review_type == prediction, "1", "0")) %>%
  rowid_to_column("document") %>%
  mutate(feature_type = "bigram+unigram")

test_preds_random_all <- bind_rows(test_preds_random, test_preds_random2)

prec_random <- ggplot(test_preds_random_all, aes(prediction, fill = correct)) +
  geom_bar(position = "fill") +
  facet_grid(rows = "feature_type")

recall_random <- ggplot(test_preds_random_all, aes(review_type, fill = correct)) +
  geom_bar(position = "fill") +
  facet_grid(rows = "feature_type") +
  theme(axis.title.y=element_blank(),
        axis.text.y=element_blank(),
...
ggsave(plot_grid(prec_random + theme(legend.position = "none"), 
                 recall_random + theme(legend.position = "none"),
                 legend, nrow = 1, rel_widths = c(5, 4, 1)), 
       filename = "confplotsrandom.png", width = 6, height = 4)

conf_random2 <- table(test_labels, predicted_random2)
conf_random2
...
perf_random2 <- performance(conf_random2)
perf_random2 %>% kable()
...
most_important5 <- tibble(`Unigram mutual information` = mi_top5$term,
                          `Unigram+bigram Mutual information` = mi_top52$term,
                          `Unigram glm` = glm_top5$term,
                          `Unigram+bigram glm` = glm_top52$term)

most_important5 %>% kable()

bind_cols(glm_top5_deceptive %>% 
            rename(`Top 5 unigram`=term) %>%
            rename(`Coefficient unigram`=coefficient),
          glm_top52_deceptive %>%
            rename(`Top 5 bi+unigram`=term) %>%
            rename(`Coefficient bi+unigram`=coefficient))
bind_cols(glm_top5_truthful %>% 
            rename(`Top 5 unigram`=term) %>%
            rename(`Coefficient unigram`=coefficient),
          glm_top52_truthful %>%
            rename(`Top 5 bi+unigram`=term) %>%
...
bind_cols(mi_top5_deceptive %>% 
            rename(`Top 5 unigram`=term) %>%
            rename(`mi unigram`=mutual_information),
          mi_top52_deceptive %>%
            rename(`Top 5 bi+unigram`=term) %>%
            rename(`mi bi+unigram`=mutual_information))
bind_cols(mi_top5_truthful %>% 
            rename(`Top 5 unigram`=term) %>%
            rename(`mi unigram`=mutual_information),
          mi_top52_truthful %>%
            rename(`Top 5 bi+unigram`=term) %>%
...
perf_compare <- tibble(metric = perf_mnb[["metric"]],
                       mnb_mi = perf_mnb[["value"]],
                       mnb2_mi = perf_mnb2[["value"]],
                       mnb_fq = perf_mnb_freq[["value"]],
                       mnb2_fq = perf_mnb2_freq[["value"]],
                       glm = perf_glm[["value"]],
                       glm2 = perf_glm2[["value"]],
                       tree_mi = perf_tree[["value"]],
                       tree2_mi = perf_tree2[["value"]],
                       tree_fq = perf_tree_freq[["value"]],
                       tree2_fq = perf_tree2_freq[["value"]],
                       rand = perf_random[["value"]],
                       rand2 = perf_random2[["value"]])

correct_counts <- perf_compare %>%
  filter(metric=="accuracy") %>%
  gather(key = models, value = accuracy, -metric) %>%
  mutate(correct = accuracy * length(test_labels),
         incorrect = length(test_labels) - correct,
         models = factor(models)) %>%
  select(models, incorrect, correct)

correct_uncount <- uncount(correct_counts, correct) %>%
  mutate(response = 1) %>% select(-incorrect)

incorrect_uncount <- uncount(correct_counts, incorrect) %>%
  mutate(response = 0) %>% select(-correct)

complete_uncount <- bind_rows(correct_uncount, incorrect_uncount) %>%
  rownames_to_column("id")

correct_bar <- correct_counts %>%
  gather(key = "type", value = "count", -models) %>%
  ggplot() +
  geom_bar(aes(models, count, fill = factor(type, levels = c("incorrect", "correct"))), stat = "identity") + 
  theme(legend.title = element_blank(), axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))


ggsave(filename = "correctbar.png", plot = correct_bar, height = 3)
...
predictions_per_model <- tibble(predictions = c(predicted_mnb, 
                                      ...
                                      as.vector(predicted_glm2), 
                                      predicted_tree,
                                      predicted_tree2,
                                      predicted_tree_freq,
                                      predicted_tree2_freq,
                                      predicted_random,
                                      predicted_random2),
                      models = c(rep("mnb unigram mi", 
                                     length(predicted_mnb)), 
                                 rep("mnb unigram and bigram mi", 
                                     length(predicted_mnb)), 
                                 rep("mnb unigram freq", 
                                     length(predicted_mnb)), 
                                 rep("mnb bigram freq", 
                                     length(predicted_mnb)), 
                                 rep("logistic regression unigram freq", 
                                     nrow(predicted_glm)), 
                                 rep("logistic regression bigram freq", 
                                     nrow(predicted_glm)),
                                 rep("single tree unigram mi", 
                                     length(predicted_tree)),
                                 rep("single tree bigram mi", 
                                     length(predicted_tree2)),
                                 rep("single tree unigram freq", 
                                     length(predicted_tree)),
                                 rep("single tree bigram freq", 
                                     length(predicted_tree2)),
                                 rep("random forest unigram freq",
                                     length(predicted_random)), 
                                 rep("random forest bigram freq",
                                     length(predicted_random))), 
                      ground_truth = c(rep(test_labels, 12)), 
...
head(complete_uncount, 10)

length(complete_uncount$response)
length(complete_uncount$models)

#pairwiseMcnemar(response ~ models,
#                     data   = complete_uncount,
#                     test   = "permutation",
#                     method = "fdr",
#                     digits = 3)

mcnemartest <- pairwiseMcnemar(x=complete_uncount$response, 
                g=complete_uncount$models, 
                block = complete_uncount$id,
                method="fdr", 
                test="mcnemar")

tibble(mcnemartest$Pairwise) %>%
  filter(p.value<0.05) %>%
  kable()
